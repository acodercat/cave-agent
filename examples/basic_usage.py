...
async def main():
    ...
    runtime1 = IPythonRuntime(functions=[add_func, multiply_func])
    ...
    response = await agent2.run(
        "Use processor to sort the numbers and store the result in the 'result' variable"
    )
    ...
    runtime3 = IPythonRuntime(functions=[Function(add), Function(multiply)])
    agent3 = CaveAgent(model, runtime=runtime3)

    # Stream events — frozen dataclasses, matched on type
    async for event in agent3.stream_events("Calculate 10 plus 20, then multiply the result by 3"):
        match event:
            ...
            case ExecutionResultEvent():
                print(f"[{'output' if event.success else 'error'}] {event.output.strip()}")
            case StoppedEvent():
                print(
                    f"\n[stopped] {event.stop_reason} · {event.steps} steps "
