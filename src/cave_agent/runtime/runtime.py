...
from ..prompts import SKILLS_INSTRUCTION
from ..skills import Skill, SkillRegistry
from .builtins import activate_skill
...
class Runtime:
    ...
    def __init__(
        ...
        Args:
            ...
            skills: List of skills to register with this runtime
        """
    ...
    def has_function(self, name: str) -> bool:
        """Check whether a function with the given name is already registered."""
    ...
    def describe_skills(self) -> str:
        ...
        return self._skill_registry.describe_skills()

    @property
    def skill_instructions(self) -> str:
        """System-instruction text to append when skills are registered.

        Empty string when no skills; SKILLS_INSTRUCTION block otherwise.
        """
        return "\n" + SKILLS_INSTRUCTION if self._skill_registry.list_skills() else ""

    def _inject_skills(self) -> None:
        """Inject ``_skill_store`` and ``activate_skill`` into the executor namespace.

        No-op when the registry is empty. Idempotent — safe to call after ``reset()``.
        """
        if not self._skill_registry.list_skills():
            return
        store = self._skill_registry.build_skill_store()
        self._executor.inject_into_namespace("_skill_store", store)
        if "activate_skill" not in self._functions:
            self.inject_function(Function(activate_skill))
    ...
    async def reset(self):
        ...
        self._types.clear()
        # Skills survive reset — re-inject the store into the fresh namespace.
        self._inject_skills()
