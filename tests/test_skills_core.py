...
class TestAgentSkillsInit:
    def test_runtime_with_skills_list(self, mock_model):
        skill = Skill(name="test-skill", description="Test")
        runtime = IPythonRuntime(skills=[skill])
        agent = CaveAgent(model=mock_model, runtime=runtime)
        assert any(s.name == "test-skill" for s in agent.runtime.list_skills())

    def test_runtime_with_skills_from_discovery(self, temp_dir, mock_model):
        (temp_dir / "SKILL.md").write_text(
        ...
        agent = CaveAgent(model=mock_model, runtime=runtime)
        assert any(s.name == "test" for s in agent.runtime.list_skills())

    def test_agent_no_skills(self, mock_model):
        agent = CaveAgent(model=mock_model)
        assert agent.runtime.list_skills() == []

    def test_reusing_runtime_across_agents(self, mock_model):
        """Two agents over the same runtime share the same skill set — no crashes, no duplicates."""
        skill_a = Skill(name="skill-a", description="First skill")
        skill_b = Skill(name="skill-b", description="Second skill")
        runtime = IPythonRuntime(skills=[skill_a, skill_b])

        agent1 = CaveAgent(model=mock_model, runtime=runtime)
        agent2 = CaveAgent(model=mock_model, runtime=runtime)

        # Both agents see the same runtime-owned skill set.
        assert agent1.runtime.describe_skills() == agent2.runtime.describe_skills()
        # activate_skill was injected exactly once.
        assert runtime.has_function("activate_skill")
        assert len([f for f in runtime._functions if f == "activate_skill"]) == 1
...
class TestAgentSkillsIPyKernel:
    ...
    def test_activate_skill_function_injected(self, mock_model):
        skill = Skill(name="test", description="Test")
        runtime = IPyKernelRuntime(skills=[skill])
        agent = CaveAgent(model=mock_model, runtime=runtime)
        assert "activate_skill" in agent.runtime.describe_functions()
